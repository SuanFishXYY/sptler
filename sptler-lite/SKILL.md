...
- **落地段的可执行要点由"必须追问"驱动**——把圣人的追问（如王升"骨架是什么/边界在哪"）转成具体动作（"补XX骨架""钉死XX边界"），不是凭空编要点。**转化范式**：每个追问→1条要点（追问"位置如何定义"→要点"在独权中定义AOA角度估计的位置关系：'基于蓝牙AOA信号'限定信号来源，不限定算法"）。要点须直接回答追问，不是绕开。
...
2. **追问驱动要点没**：每条可执行要点是否直接回答了圣人的一个"必须追问"（追问"位置如何定义"→要点"在独权中定义AOA角度估计的位置关系"），而非凭空编？且具体到动词+对象，非"注意边界"空话？若追问未被任何要点回答 → 补一条直接回答追问的要点。
...
## 四、风险与收口
风险：{1句}。[邹蕴] {1句总结}。
**议会自省**：{邹蕴1句评议会本身——规模/匹配/流程轻重}。议会到此结束。
```
