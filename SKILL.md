...
- **Briefing** (`/sptler!`): 3-5 sages, no AskUser, concise output. See runtime.md §Briefing.
...
New users: auto-proceed. Experienced: confirm + invite. Detail: runtime.md §Phase 0.5.

### Phase 1 — Routing + memory injection (10 steps)
route_sages → summon_sage --dry-run → watch_memory → scenario data collection → auto_invite. Detail: runtime.md §Phase 1.
...
8. **Run all phases** (fast/formal) — verdict/briefing are the only exceptions.
...
14. **邹蕴 must conclude** — substantive conclusion (decision + reason + risk); if user can't tell what was decided, parliament failed.
