...
| **简报** | `/sptler! {问题}` | ≤5（脚本硬上限） | 中等议题、要深度但不想被 AskUser 打断 | `route_sages --briefing` 强制 fast、免提问、单轮、场景走快评不拒绝 |
...
**精简模式独立入口 `/sptler-lite`**（可选，省 token 快车道）：

```bash
# sptler-lite 是独立 skill，复用 sptler 的脚本/圣人/references，只装极薄 SKILL.md
cp -r ~/.claude/skills/sptler/sptler-lite ~/.claude/skills/sptler-lite
```

装后即可 `/sptler-lite {问题}`（等价于 `/sptler#`）。sptler 更新后 sptler-lite 自动获得最新脚本，无需单独更新。
