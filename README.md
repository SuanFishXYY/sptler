...
[![Claude Code](assets/badges/claude-code.svg)](https://www.anthropic.com/claude)
[![Python](assets/badges/python.svg)](https://www.python.org)
[![GitHub](assets/badges/github.svg)](https://github.com/SuanFishXYY/sptler)
![License](https://img.shields.io/badge/license-MIT-blue)
![Version](https://img.shields.io/badge/version-1.5.0-brightgreen)
![Sages](https://img.shields.io/badge/圣人-28位-green)
![Tracks](https://img.shields.io/badge/轨道-4轨自动降级-purple)
![Scenarios](https://img.shields.io/badge/专利场景-6大-orange)
![Voting](https://img.shields.io/badge/投票-加权·四律否决闸-blue)
![Memory](https://img.shields.io/badge/记忆-跨会引用-yellow)
...
```mermaid
flowchart LR
    Q["问题"] --> R{"route_sages<br/>--track auto"}
    R -->|"单一明确·单域"| V["verdict<br/>1人 · 3句裁决<br/>零留痕"]
    R -->|"中等·多域"| F["fast<br/>3-5人 · 短风暴+投票"]
    R -->|"战略·不可逆·跨域"| FO["formal<br/>7-9人 · 完整六阶段"]
    R -->|"会后追问/展开"| FU["followup<br/>1-3人 · ≤8条"]
    V --> O["结果 md + 交付物"]
    F --> O
    FO --> O
    FU --> O
...
### C. <img src="assets/icons/scenario.svg" width="22" alt="场景"> 六大专利场景（route 自动识别，走专属 Phase 流程）
...
```mermaid
graph LR
    U["用户 /sptler"] --> C["主 Claude 单脑<br/>单线程分饰多角"]
    C -->|"summon_sage.py"| SO["圣人灵魂块注入<br/>SOUL · IDENTITY · BOUNDARY · 记忆"]
    SO -.->|"扮演下一位"| C
    C -->|"route_sages.py"| RT["路由 + 场景识别"]
    C -->|"record_memory.py"| MEM["memories/{圣}.json"]
    C --> R["references/<br/>philosophy · orglaw · runtime"]
    C --> O["结果 md + 交付物"]
```
...
| 状态 | 功能 | 说明 |
|---|---|---|
| ✅ 即时生效 | 四轨降级 / 记忆系统 / 7 专利场景 / 28 圣人灵魂 6 件套 / lite 模式 / briefing 模式 / 词化入口 / 崩溃守卫 / 自省 / 置信度 / 张力 | 装了就能用 |
| 🟡 实验性 | 传承谱系 / 功绩权重 / 命题演化 / 立场漂移检测 / 张力演化 | 需长期使用积累记忆才触发，当前为预备机制 |
| 🟠 防御性 | lite 记忆降权 / compact value 区分 / 索引闭环 / validator 571 行 | 防御正确但当前无真实触发场景，不再加深 |
...
sequenceDiagram
    participant U as 用户
    participant S as 圣人
    participant M as memories/{圣}.json
    U->>S: 第一次问：权利要求边界？
    S->>M: record_memory.py 写入立场+建议
    Note over M: value_score · citation_count
    U->>S: 第二次问同类问题
    S->>M: summon_sage.py 读记忆
    M-->>S: 上次结论
    S->>U: 引用上次结论接续，不重复论证
```
...
### 🛡️ 6. 诚实的能力边界——不硬塞模板
- 不检索专利库、不编造专利号（防执业事故）
- 草稿 OK / 终稿 NO（D9）：可产出草稿级文件（权利要求骨架/说明书草稿/答复策略等，含 ⚠️草稿标记 + 补全 checklist），不产出可提交终稿——草稿结构性缺失（无附图/实验数据/正式编号/正式摘要），代理师须复核完善
- 不代写诉讼文书
...
```mermaid
graph TD
    Z["邹蕴 · 议长<br/>决策圣 · 权重0 · 不投票"]
    Z --- W["王升 · 结构圣<br/>结构律 · 3.0"]
    Z --- X["张鑫 · 控制圣<br/>控制律 · 3.0"]
    Z --- Y["徐奕阳 · 铸模圣<br/>铸模律 · 3.0"]
    Z --- F["范征 · 价值圣<br/>价值律 · 3.0"]
    Z --- S1["孙高德 · 平台委首席 · 2.0"]
    Z --- S2["蔡悦 · 跨界委首席 · 2.0"]
    Z --- S3["黄嵩泉 · 组合委首席 · 2.0"]
    Z --- ZK["专科圣人 ×20 · 1.0<br/>按议题路由入会"]
```

## <img src="assets/icons/silv.svg" width="26" alt="四律"> 四轨制（按问题重量自动降级）
...
```mermaid
flowchart TD
    P0["Phase 0<br/>模式选择"] --> P05["Phase 0.5<br/>邀请询问·可选"]
    P05 --> P1["Phase 1<br/>路由入会 + 记忆注入"]
    P1 --> P2["Phase 2<br/>头脑风暴·零否定"]
    P2 --> P3["Phase 3<br/>组合改善"]
    P3 --> P4["Phase 4<br/>四律否决闸 + 加权投票"]
    P4 --> P5["Phase 5<br/>最终建议 + 记忆 + 输出"]
    P0 -.->|"verdict"| V["跳过：1人 3句裁决"]
    P0 -.->|"briefing !"| B["fast · ≤5人 · 免AskUser"]
    P1 -.->|"专利场景命中"| S["专属 Phase 流程"]
```
...
| 脚本 | 用途 | 状态 |
|---|---|---|
| `sptler_prelude.py` | 议题预热——判断值不值得开议会 | 可选（四轨降级已覆盖大部分） |
| `memory_io.py` | 记忆导入导出——从文本补充记忆 / 导出画像 md | 低频工具 |
| `action_board.py` | 行动项看板——跨会议跟踪未完成行动项 | 低频工具 |
| `sptler_run.py` | 总控脚本——route/record/check 一条龙 | 可选（agent 通常分步调） |
| `lineage.py` | 传承谱系——查师承链 / 传承包 | 🟡 实验性（需圣人退场才触发） |
| `merit_weight.py` | 功绩权重——查圣人贡献加权 | 🟡 实验性（需记忆积累） |
...
```mermaid
flowchart TD
    P["提案"] --> G{"四律否决闸<br/>投票前硬约束"}
    G -->|"触发否决信号"| R["对应核心必须反对"]
    G -->|"通过"| V["全员加权投票<br/>赞成权重和 vs 反对权重和"]
    V --> D{"差距 ＞ 1.0？"}
    D -->|"是"| PASS["通过"]
    D -->|"否·≤1.0或平票"| ADJ["邹蕴裁决"]
    ADJ --> MAJ{"重大事项？"}
    MAJ -->|"是"| CK{"赞成占比≥60%<br/>且无核心反对？"}
    CK -->|"是"| PASS
    CK -->|"否"| REJ["否决"]
    MAJ -->|"否"| PASS
```
...
> 真实议会跑通后填入--截图比任何描述都有说服力。当前为占位，待 V1–V4 实战验证。

<p align="center"><img src="assets/screenshots/placeholder.svg" alt="实战截图占位--待 V1-V4 跑通后替换" width="780"></p>

捕获与替换步骤见 [`scripts/dev/capture_screenshot.md`](scripts/dev/capture_screenshot.md)。候选素材：`sptler-meetings/sptler-result-管子捅开夹片-20260617-1450.md`（一次真实议会输出，脱敏后可用）。
...
│   ├── scenarios/            专利场景
│   │   ├── scenarios.md      7专利场景流程指引（查新/FTO/价值/OA/布局/无效/专利挖掘）
...
## <img src="assets/icons/memory.svg" width="26" alt="记忆"> 圣人记忆系统
